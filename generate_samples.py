		...
		for i in range(len(para["texts_gloves"])):
			...
			for before_state, after_state in zip(before_states, after_states):
				if before_state == after_state:
					...
					state_labels.append("create")
					spans.append(get_span(para["texts"][i], after_state))
				elif after_state == "-":
					state_labels.append("destroy")
					spans.append(get_span(para["texts"][i], before_state))
				else:
					state_labels.append("move")
					spans.append((get_span(para["texts"][i], before_state), get_span(para["texts"][i], after_state)))
